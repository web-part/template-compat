...
*
* source md5: BC1C595751A67BBD8FC7D68A1958207C
*
* source file: htdocs/lib/table/Table.js
*/
...
define('Table', function (require, module, exports) {
    ...
    */
    ...
    Table.prototype = {
        ...
        * 已重载 move(row, step);
        */
        move: function move(item, step) {
            var meta = mapper.get(this);
            var type = typeof item === 'undefined' ? 'undefined' : _typeof(item);

            var row = type == 'number' ? meta.rows[item] : type == 'string' ? meta.id$row[item] : type == 'object' ? meta.id$row[item.id] : null;
        ...
        */
        get: function get(key) {
            ...
            if (typeof key == 'string') {
                return meta.id$cell[key] || meta.id$row[key] || meta.id$column[key];
            }
        },
        ...
        column: function column(name, fn) {
            var meta = mapper.get(this);
        ...
        */
        reaction: function reaction(name, deps, change) {
            var _Reaction$set;

            var meta = mapper.get(this);
            (_Reaction$set = Reaction.set).call.apply(_Reaction$set, [meta.name$column].concat(Array.prototype.slice.call(arguments)));
        },
